...
Single-worker task execution is delivered. M3 descriptor, event/reducer, path/workspace, durable-store, execution-host, and recovery-coordinator contracts are implemented in the review stack. The current CLI does not compose them or expose pause, cancel, and resume yet. Parallel graphs, approvals, and additional runtime integrations remain roadmap work; see [milestones](../../docs/MILESTONES.md).
