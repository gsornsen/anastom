...
const roots: string[] = [];
afterEach(async () => { vi.useRealTimers(); vi.unstubAllEnvs(); for (const root of roots.splice(0)) await rm(root, { recursive: true, force: true }); });
async function workspace(): Promise<WorkspaceRef> {
  const path = await mkdtemp(join(tmpdir(), "anastom-command-")); roots.push(path);
...
function command(script: string, extra: Partial<CommandDefinition> = {}): CommandDefinition {
  return { argv: [process.execPath, "-e", script], cwd: ".", maxDurationMs: 2000, maxOutputBytes: 4096, ...extra };
}
describe("control-plane commands", () => {
  it("passes exact arguments without a shell and omits provider credentials", async () => {
    ...
    const run = await new LocalCommandExecutor().execute(command("process.stdout.write(process.argv[1]); process.stderr.write(String(process.env.ANASTOM_TEST_SECRET))", {
      argv: [process.execPath, "-e", "process.stdout.write(process.argv[1]); process.stderr.write(String(process.env.ANASTOM_TEST_SECRET))", "$HOME; echo surprise"],
    }), await workspace());
  ...
  it("preserves output and reports a nonzero exit or unavailable executable", async () => {
    ...
    const result = await executor.execute(command("console.log('evidence'); console.error('failure'); process.exit(7)"), target);
    ...
    expect((await executor.execute(command("", { argv: ["anastom-nonexistent-executable"] }), target)).failure?.category).toBe("tool");
  });
  it("drains excess output while bounding retained bytes per stream", async () => {
    const result = await new LocalCommandExecutor().execute(command("process.stdout.write('x'.repeat(100000)); process.stderr.write('y'.repeat(50000))"), await workspace());
    expect(result.output).toMatchObject({ passed: true, stdoutBytes: 100000, stderrBytes: 50000, stdoutTruncated: true, stderrTruncated: true });
  ...
  it("terminates a process that ignores SIGTERM after a bounded grace", async () => {
    const target = await workspace(); if (target.mode === "memory") throw new Error("Expected filesystem fixture");
    vi.useFakeTimers({ toFake: ["setTimeout", "clearTimeout"] });
    const ready = join(target.path, "ready");
    const running = new LocalCommandExecutor(20).execute(command("process.on('SIGTERM', () => {}); require('node:fs').writeFileSync(" + JSON.stringify(ready) + ", 'ready'); setInterval(() => {}, 1000)", { maxDurationMs: 1_000_000 }), target);
  ...
  it("rejects lexical and symlink cwd escapes, empty argv, and invalid limits", async () => {
    const target = await workspace();
    if (target.mode === "memory") throw new Error("Expected filesystem fixture");
    await symlink(tmpdir(), join(target.path, "escape"), "dir");
    const executor = new LocalCommandExecutor();
    for (const cwd of ["..", "/", "escape"]) await expect(executor.execute(command("void 0", { cwd }), target)).rejects.toThrow();
    await expect(executor.execute(command("", { argv: [] }), target)).rejects.toThrow("argument");
    await expect(executor.execute(command("void 0", { maxDurationMs: 0 }), target)).rejects.toThrow("limits");
  });
});
