...
async function setup() {
  const root = await mkdtemp(join(tmpdir(), "anastom-sqlite-")); roots.push(root);
  ...
  const engine = new WorkflowEngine({ runtime: new FakeRuntimeAdapter({ nodes: {} }), persistence: store });
...
describe("SQLite run store", () => {
  ...
  it("rejects sequence conflicts and rolls back invalid event batches", async () => {
    ...
    await store.append("persisted", seq, [{ type: "AttemptScheduled", runId: "persisted", sequence: seq + 1, nodeId: "analyze", attempt: 1, runtimeId: "fake" }]);
    await expect(other.append("persisted", seq, [])).rejects.toThrow("expected");
    const saved = (await store.load("persisted"))!;
    await expect(store.append("persisted", seq + 1, [
      { type: "AttemptStarted", runId: "persisted", sequence: seq + 2, nodeId: "analyze", attempt: 1 },
      { type: "NodeSucceeded", runId: "persisted", sequence: seq + 3, nodeId: "analyze" },
    ])).rejects.toThrow();
    expect(await store.load("persisted")).toEqual(saved);
    await expect(store.create("persisted", { workflow, events: run.events })).rejects.toThrow("already exists");
  ...
  it("rejects a gap or invalid event payload on replay", async () => {
    ...
    const raw = new DatabaseSync(path);
    raw.exec("DROP TRIGGER immutable_events_update");
    const event = { type: "NodeReady", runId: "persisted", sequence: 2, nodeId: "analyze", reason: "invented" };
  ...
  it.each(["missing", "gap", "json", "wrong-run"])("rejects %s history rather than guessing state", async (damage) => {
    const { path, store } = await setup(); const raw = new DatabaseSync(path);
    raw.exec("DROP TRIGGER immutable_events_update; DROP TRIGGER immutable_events_delete");
    if (damage === "missing") raw.exec("DELETE FROM events");
    if (damage === "gap") raw.exec("UPDATE events SET sequence=3 WHERE sequence=2");
    if (damage === "json") raw.exec("UPDATE events SET event_json='{' WHERE sequence=1");
    if (damage === "wrong-run") raw.exec("UPDATE events SET event_json='{\"type\":\"RunCreated\",\"runId\":\"different\",\"sequence\":1}' WHERE sequence=1");
...
describe("filesystem artifacts", () => {
  it("stores immutable bytes and detects tampering", async () => {
    const { root, store } = await setup(); store.close();
    const artifacts = new FileArtifactStore(root);
    const input = { runId: "persisted", nodeId: "work", attempt: 1, type: "diff", mediaType: "text/plain", bytes: "original" };
