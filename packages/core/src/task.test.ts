...
import { canonicalJson, digestJson, loadTask, parseTaskMarkdown, compileTask, assertWorkflowDefinition, parseWorkflowYaml, normalizeWorkflow } from "./index.js";
import { taskPath } from "../../engine/src/testing/fixture.js";

describe("M1 task descriptor", () => {
  ...
  it("rejects unknown fields, missing objective, unsupported modes, and shell strings", async () => {
    ...
    expect(() => parseTaskMarkdown(source.replace("kind: Task", "kind: Task\nsecret: ignored"))).toThrow("secret");
    expect(() => parseTaskMarkdown(source.split("\n# Add")[0] + "\n")).toThrow("objective");
    expect(() => parseTaskMarkdown(source.replace("mutation: isolated", "mutation: shared-integration"))).toThrow();
    expect(() => parseTaskMarkdown(source.replace("argv: [node, --test, server.test.mjs]", "argv: node --test"))).toThrow();
    await expect(compileTask(parseTaskMarkdown(source.replace("maxDuration: 30s", "maxDuration: 30s\n  cwd: ../outside")), taskPath)).rejects.toThrow("workspace");
    await expect(compileTask(parseTaskMarkdown(source.replace("maxDuration: 5m", "maxDuration: 999999999999h")), taskPath)).rejects.toThrow("timer range");
  });
  it("rejects invalid normalized graph and embedded schemas on reload", async () => {
    ...
    expect(() => assertWorkflowDefinition({ ...workflow, nodeOrder: ["constructor", "verify"] })).toThrow("node order");
  });
  it("applies worker policy defaults independently and owns its schema copies", async () => {
    ...
    const first = await compileTask(parseTaskMarkdown(partial), taskPath);
    const second = await compileTask(parseTaskMarkdown(partial), taskPath);
  ...
  it("preserves M0 syntax while enforcing M1 field ownership", async () => {
    ...
    expect(() => parseWorkflowYaml(base.replace("kind: command", "kind: command, needs: [constructor]"))).toThrow("unknown node");
    expect(() => parseWorkflowYaml(base.replace("kind: command", "kind: command, mutation: isolated"))).toThrow("agent");
    expect(() => parseWorkflowYaml(base.replace("kind: command", "kind: agent, role: worker, command: {argv: [echo], maxDuration: 1s}"))).toThrow("command nodes");
    const doc = parseWorkflowYaml(base.replace("kind: command", "kind: command, command: {argv: [echo, '$HOME'], maxDuration: 1s}"));
    const workflow = await normalizeWorkflow(doc, { sourcePath: "/tmp/workflow.yaml", loadSchema: async () => ({ type: "object" }) });
