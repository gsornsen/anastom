...
function request(attempt = 1): ExecutionRequest {
  return {
    runId: "pi-test",
    workflowInstanceId: "pi-test:root",
    nodeId: "implement",
    nodeKind: "agent",
    attempt,
    workspace: {
    ...
    context: {
      ...
      task: {
        ...
        acceptanceCriteria: ["health responds"],
      },
    },
    budget: { maxAttempts: 2, maxDurationMs: 1000 },
    requiredOutputSchema: workerReportSchema,
...
function session(
  ...
  const value: PiSession = {
    ...
    subscribe: vi.fn((observe: (event: RuntimeEvent) => void) => {
      listener = observe;
      return unsub;
    }),
...
async function events(adapter: PiRuntimeAdapter, handle: { id: string }) {
...
describe("Pi SDK boundary without model calls", () => {
  it("makes a new ephemeral session per attempt, emits public metadata, and disposes each one", async () => {
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      ...
      expect(sessions[attempt - 1]!.value.prompt).toHaveBeenCalledWith(
        renderPiPrompt(request(attempt)),
      );
  ...
  it("sanitizes provider failures and never maps private reasoning or tool bodies", async () => {
    ...
    expect(observablePiEvent(tool)).toEqual({
      type: "log",
      message: "Pi tool finished: bash (failed)",
    });
  });
  it("aborts once across concurrent cancellation and closes the stream", async () => {
    ...
    expect((await events(adapter, handle)).at(-1)).toEqual({
      type: "completed",
      status: "cancelled",
    });
    expect(sdk.value.dispose).toHaveBeenCalledOnce();
  ...
  it("honors cancellation during initialization and does not prompt", async () => {
    const sdk = session();
    let initialized!: (value: PiSession) => void;
    const adapter = new PiRuntimeAdapter({
      factory: async (_request, signal) => {
    ...
    const handle = await adapter.start(request());
    const cancelled = adapter.cancel(handle);
    initialized(sdk.value);
    await cancelled;
    expect(sdk.value.prompt).not.toHaveBeenCalled();
    expect(sdk.value.abort).toHaveBeenCalledOnce();
    expect(sdk.value.dispose).toHaveBeenCalledOnce();
  });
