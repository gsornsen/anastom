...
**Next: M2's portable-worker implementation is in progress.** The feature branch adds selected-runtime capability checks, normalized identity and available-token observations, a pinned Codex CLI adapter, and shared deterministic conformance tests. The Codex adapter passed offline provider/process fixtures and an owner-authenticated paired Pi/Codex endpoint demonstration; final CI and the full [M2 completion evidence](docs/M2_EVIDENCE.md) remain under review. M1 is the completed support baseline.
