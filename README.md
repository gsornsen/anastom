...
- fenced durable-run storage plus a shared two-phase POSIX execution host with authenticated bounded IPC, parent-death cleanup, and fail-closed supervisor-loss inspection;
...
| **M3 — Durable execution**       | Building | Feasibility passes on Linux/macOS; runtime reconstruction, recovery events, private paths, workspace checkpoints, the fenced store, and shared execution ownership are implemented while recovery coordination and CLI layers remain. |
