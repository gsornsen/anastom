...
**Current stage: M2.5 is complete, and M3 durable-execution feasibility is active under an accepted design.** Anastom can run the same strict Markdown Task through Pi, an exact-pinned Codex CLI worker, or an attested Claude Code CLI worker; check the selected runtime's capabilities before work starts; independently verify the result; and replay public identity and available token observations from durable history. The unchanged endpoint Task passed through all three owner-authenticated harnesses. M3 recovery commands are not product features yet. See the [M2 completion evidence](docs/M2_EVIDENCE.md), [M2.5 completion evidence](docs/M2_5_EVIDENCE.md), and [M1 compatibility baseline](docs/M1_EVIDENCE.md).

The [offline CLI audit](docs/M2_FEASIBILITY.md) led to an [approved discovery/authentication profile](docs/adr/0012-codex-discovery-and-authentication-profile.md). An accumulated-context fixture reproduced native compaction, leading to the [accepted catalog control](docs/adr/0013-codex-client-compaction-profile.md). A managed-policy fixture then found hidden instructions despite native success; [accepted ADR 0014](docs/adr/0014-codex-managed-policy-preflight.md) adds a model-free policy gate before Codex execution. M2.5 added the separately installed Claude Code CLI with explicit subscription/API-key selection and fail-closed native, authentication, and managed-policy checks. This release supports one selected worker and one verifier. The accepted [M3 durability contract](docs/M3_BUILD_BRIEF.md) defines the ownership, orphan, workspace, snapshot, pause, cancel, and resume boundary. [M3 feasibility](docs/M3_FEASIBILITY.md) rejects current handles and direct leader PIDs as recovery identities. Its shared parent-death supervisor passes on Linux and macOS; a production-shaped successor proves two-phase start authorization, bounded authenticated IPC, public result relay, confirmed cleanup, and fail-closed supervisor loss across all four current execution owners on the owner macOS host and Ubuntu/macOS CI. A multi-process SQLite probe proves the proposed lease generations, stale fencing, idempotent mutation ranges, and deduplicated control inbox on the same platforms. Exact workspace evidence covers committed, staged, tracked, untracked, binary, symlink, ignored-content, and canonical ownership changes while preserving staging on Linux and macOS. Local snapshot evidence now proves snapshot-plus-tail equality, authoritative prefix binding, and corrupt-snapshot fallback; cross-platform confirmation and separate production API review remain.
...
| **M3 — Durable execution**       | Feasibility | Process, SQLite, and workspace evidence pass cross-platform; snapshot evidence passes locally; production APIs remain unfrozen.                        |
...
See the [full milestones](docs/MILESTONES.md), [M1 build brief](docs/M1_BUILD_BRIEF.md), [M2 build brief](docs/M2_BUILD_BRIEF.md), [M2.5 build brief](docs/M2_5_BUILD_BRIEF.md), accepted [M3 build brief](docs/M3_BUILD_BRIEF.md), and [changelog](CHANGELOG.md) for scope and progress.
