...
- dependency scheduling with stable ordering for static workflows and bounded concurrent attempts for durable expanded graphs;
...
- a repository-bound Feature CLI that snapshots `sdlc/default`, protects authored policy inputs, creates the run integration topology, and streams the same durable public projection used for later inspection;
...
Pi uses your normal provider authentication and model settings; its Git worktree is checkout isolation, not an operating-system sandbox. Codex uses its normal file-backed CLI authentication, requires an explicit OpenAI model, and runs its worker commands in read-only or workspace-write sandboxing. Claude Code uses an end-user-installed, signed native release with an explicit first-party subscription or API-key source. Codex and Claude Code reject managed policy that their bounded profiles cannot certify. The independent verifier runs with local command permissions. Run trusted Feature/Task inputs and verification commands on repositories you are comfortable exposing to the selected provider. M3's local crash/restart acceptance passes without a provider call, and the completed implementation passes Linux, macOS, and repository-required checks. Human approval, heterogeneous role routing, and further adapters remain future work.
...
M4 remains under development. Its [model-free feasibility evidence](docs/M4_FEASIBILITY.md) establishes stable graph expansion, multiple execution identities under one fence, and recoverable deterministic Git integration. The Feature/methodology/plan/review contracts, executable expansion events, concurrent durable scheduling and recovery, task-worktree assignment, scoped patch acceptance, deterministic wave/integrator commits, independent-review semantics, operator-verifier authority, derived evidence ledger, bounded reconnect-equivalent public live stream, and production Feature CLI composition are implemented. The [production-shaped process evidence](docs/M4_EVIDENCE.md) covers both parallel completion orders, whole-run controls, failure gates, later-process inspection, immutable artifacts, retained branches, and interrupted-integration recovery without model calls. Live Pi/Codex acceptance remains in the active stack.
...
To exercise the complete defined-SDLC process matrix without a model or provider credentials, run:
...
```bash
pnpm anastom run feature.md --method sdlc/default --runtime pi
pnpm anastom run feature.md --method sdlc/default --runtime codex --provider openai --model YOUR_CODEX_MODEL --reasoning-effort medium
```
...
| **M4 — Defined SDLC**            | Building | Plan, delegate parallel work, integrate, review, and verify a feature.                                                                                 |
| **M4.5 — Terminal experience**   | Planned  | Follow live workflow state, bounded public runtime output, evidence, and controls through an interactive terminal.                                     |
...
M1 persistence enables inspection from later processes. M3 adds fenced recovery for an attempt that was running during a coordinator crash. [M4 issue #37](https://github.com/gsornsen/anastom/issues/37) tracks the defined-SDLC implementation, and [terminal-experience issue #38](https://github.com/gsornsen/anastom/issues/38) records the MLP interaction work planned after M4's durable event projection stabilizes.
...
| What defines the current SDLC milestone?                | [M4 build brief](docs/M4_BUILD_BRIEF.md)                                        |
