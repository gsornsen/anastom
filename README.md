...
**Current stage: M2.5 is complete, M3 durable-execution feasibility is complete, and M3 production implementation is underway.** Anastom can run the same strict Markdown Task through Pi, an exact-pinned Codex CLI worker, or an attested Claude Code CLI worker; check the selected runtime's capabilities before work starts; independently verify the result; and replay public identity and available token observations from durable history. The unchanged endpoint Task passed through all three owner-authenticated harnesses. The first M3 production slices add credential-free runtime reconstruction descriptors, exact recovery event/reducer contracts, a private operational-state path capability, and production workspace checkpoints with bounded ignored-content evidence. The durable store, execution host, recovery coordinator, and operator commands are still being built. M3 recovery is not yet a usable CLI feature. See the [M2 completion evidence](docs/M2_EVIDENCE.md), [M2.5 completion evidence](docs/M2_5_EVIDENCE.md), and [M1 compatibility baseline](docs/M1_EVIDENCE.md).
...
| **M3 — Durable execution**       | Building | Feasibility passes on Linux/macOS; runtime reconstruction, recovery events, private state paths, workspace checkpoints, and the fenced durable store are implemented while execution ownership, recovery coordination, and CLI layers remain. |
...
| Which exact APIs and records are proposed for M3?       | [M3 production contract](docs/M3_PRODUCTION_CONTRACT.md)                        |
