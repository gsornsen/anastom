...
Status: design accepted by the owner on 2026-09-16 in [PR #23](https://github.com/gsornsen/anastom/pull/23). The [M3 build brief](M3_BUILD_BRIEF.md) and [ADR 0017](adr/0017-durable-execution-ownership-and-recovery.md) settle the recovery semantics while gating exact APIs on feasibility evidence. The first [process-ownership probe](M3_FEASIBILITY.md) rejects current handles and leader-only PIDs; its shared parent-death supervisor candidate passes on Ubuntu 24.04 and macOS 15 CI in [PR #24](https://github.com/gsornsen/anastom/pull/24). A production-shaped protocol passes all four model-free owners on the owner macOS host and Ubuntu/macOS CI in stacked [PR #25](https://github.com/gsornsen/anastom/pull/25), with two-phase authorization, bounded authenticated IPC, cancellation, parent-death cleanup, and fail-closed supervisor loss. Multi-process SQLite lease, fencing, mutation-idempotency, and control-inbox contention passes on the same platforms in stacked [PR #26](https://github.com/gsornsen/anastom/pull/26), and the exact workspace-checkpoint phase passes there in stacked [PR #27](https://github.com/gsornsen/anastom/pull/27). Snapshot-plus-tail equality and corrupt-snapshot fallback pass locally with cross-platform CI pending; exact production contracts remain unfrozen.
