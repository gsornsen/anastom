...
Status: exact-release offline fixtures and adapter implementation are in progress; **M2 is not complete**. [ADR 0012](adr/0012-codex-discovery-and-authentication-profile.md) was approved in PR #15 and merged as `c5ba209`, closing [issue #14](https://github.com/gsornsen/anastom/issues/14). [ADR 0013](adr/0013-codex-client-compaction-profile.md) was approved in PR #18 and merged as `e9217e5`. [ADR 0014](adr/0014-codex-managed-policy-preflight.md) proposes a further managed-policy gate for owner review before live integration. The owner approved the original CLI direction in PR #11 at `ecd5ba5`, merged as `8659117`.
...
- bounded discovery must cover project/ancestor roots, symlinks, explicit mentions, bundled resources, and managed configuration conflicts; the managed gate is proposed in ADR 0014;
...
No M2 live model call or M2.5 implementation has been performed. A production adapter and deterministic doubles are implemented on the M2 feature branch, but do not constitute completion evidence. The approved [M2 brief](M2_BUILD_BRIEF.md) requires review of an unproven profile refinement before live integration; ADR 0014 remains subject to owner review.
