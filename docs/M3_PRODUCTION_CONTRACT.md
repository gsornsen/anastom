...
14. **The existing command result type is the host result type.** The earlier interface sketch named an undefined `CommandExecutionResult`. Production uses the existing bounded `CommandExecution` contract rather than introducing an equivalent second shape.
15. **Durable event ordering belongs to the engine.** `authorize` cannot independently query an arbitrary `DurableRunStore` without coupling infrastructure layers. The engine must durably commit `AttemptStartAuthorized` before calling it; the host then publishes an immutable authenticated start-authority record, and the supervisor cannot launch without that exact record.
...
`prepare` may start only the supervisor. It must return after the private control record and sanitized `awaiting-start` manifest are atomically published. `inspect` accepts the earlier plan reference so a replacement can classify a crash between manifest publication and `AttemptStartAuthorized`. No manifest for an exact persisted plan proves absence because launch cannot be authorized before manifest publication. The engine calls `authorize` only after `AttemptStartAuthorized` is durable under the same generation; the execution host enforces the subsequent authenticated start-authority boundary. The supervisor atomically publishes `starting` before the first worker-side effect, waits for the isolated runtime host's pre-launch readiness proof, then publishes `active` before sending launch input. Any supervisor loss from `starting` or `active` is `unknown`. `collect` resolves only after a terminal record with confirmed descendant cleanup is durable.
