...
- Add `anastom attach` with a detachable full-screen graph and public-activity view, durable pause/cancel and separately coordinated resume controls, reconnect, resize, reduced-color, deterministic plain snapshots, and JSON Lines following. Recorded and later-process acceptance reuse the defined-SDLC durable projection, human terminal output neutralizes control and bidirectional formatting characters, atomic store validation prevents a stale client from stranding a new control after terminal settlement, and explicit termination confirms forced process-group cleanup when an adapter does not settle cancellation cooperatively.
