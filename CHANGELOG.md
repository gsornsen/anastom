...
- Add a model-free M3 snapshot probe with a checked-in folded-state schema, exact workflow/event-prefix/state digest binding, snapshot-plus-tail equality across reopened lifecycle histories, newest-compatible selection, corrupt-snapshot fallback, and rejection of corrupt authoritative events. The local matrix passes; Linux/macOS CI remains before exact persistence APIs are proposed.
- Add a model-free M3 exact-workspace checkpoint probe covering committed, tracked, staged, untracked, binary, symlink, ignored-file/directory, `HEAD`, and canonical ownership changes while preserving the user's staging index. The matrix passes on Linux and macOS; snapshot feasibility remains a gate before public M3 APIs are frozen.
