...
- Add a model-free M3 exact-workspace checkpoint probe covering committed, tracked, staged, untracked, binary, symlink, ignored-file/directory, `HEAD`, and canonical ownership changes while preserving the user's staging index. The local matrix passes; Linux/macOS CI and snapshot feasibility remain gates before public M3 APIs are frozen.
